...
enum ProbeOutcome: Equatable {
    ...
    var message: String {
        ...
        case .working(let count):
            return "Connected and authorized — the service returned \(count) list\(count == 1 ? "" : "s")."
        case .reachableButUnauthorized:
...
/// wrong"; the lists route then proves the app can actually read data and says
...
///
/// The secret is REQUIRED on this path: the lists service authenticates its
...
struct ConnectionProbe {
    ...
    func run() async -> ProbeOutcome {
        ...
        do {
            ...
            // answered is not a healthy lists service. Say that here rather
        ...
        do {
            let lists = try await ListsClient(config: config, urlSession: urlSession).lists()
            return .working(listCount: lists.count)
    ...
    static func classify(apiError error: ListsError) -> ProbeOutcome {
        switch error {
        case .badURL:
            return .badURL
