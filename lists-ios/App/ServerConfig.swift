...
/// Where the app talks to the lists service, and how it authenticates.
///
///  - Simulator: straight to the service's loopback port, `127.0.0.1:4885`.
...
/// replaces a hardcoded Tailscale IP plus a `Host: lists.barry.lan` header
...
struct ServerConfig: Equatable {
    ...
    static let keychainSecretKey = "rocks.barry.lists.secret"

    static let defaultDeviceURL = "https://barry-mac.tail5cb2f2.ts.net:8448"
    static let simulatorURL = "http://127.0.0.1:4885"
    ...
    static func load() -> ServerConfig {
        // UI/integration-test hook: `-listsBaseURL <url>` overrides everything
        ...
        if let i = args.firstIndex(of: "-listsBaseURL"), args.count > i + 1 {
            var secret = ""
            if let s = args.firstIndex(of: "-listsSecret"), args.count > s + 1 {
                secret = args[s + 1]
            }
