...
final class ConnectionProbeTests: XCTestCase {
    ...
    func testRefusedCredentialIsReachableAndSaysTheSecretIsTheProblem() {
        for status in [401, 403] {
            ...
            XCTAssertFalse(outcome.isFullyWorking, "but the app still cannot load lists")
    ...
    func testSuccessReportsTheListsCount() {
        XCTAssertTrue(ProbeOutcome.working(listCount: 7).message.contains("7"),
                      "a success that names no number proves nothing about the data")
        XCTAssertTrue(ProbeOutcome.working(listCount: 7).isFullyWorking)
    }
    ...
    /// Pointing at something that answers HTTP but is NOT a healthy lists
    ...
    /// The real service with no secret. It must come back as "reachable, fix
    /// the secret" rather than as a network failure.
    ///
    /// Skips only when the service is genuinely absent — never when it answers,
    /// because an answer is the thing under test. A skip here is NOT a pass:
    /// it means the lists service was not running.
    func testRealServiceWithNoSecretReportsUnauthorized() async throws {
        ...
        guard await serviceIsUp(config) else {
            throw XCTSkip("lists service not listening on \(ServerConfig.simulatorURL)")
        }

        let outcome = await ConnectionProbe(config: config).run()
        XCTAssertEqual(outcome, .reachableButUnauthorized,
                       "the service authenticates its data routes itself, even on loopback")
    }
...
private final class StubURLProtocol: URLProtocol {
    ...
    override func startLoading() {
        ...
        client?.urlProtocol(self, didLoad: Data(#"{"ok":true,"lists":[]}"#.utf8))
