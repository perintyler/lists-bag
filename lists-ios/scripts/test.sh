#!/usr/bin/env bash
# Build and test the Lists iOS app on a simulator.
#
...
APP="$(find "$DERIVED/Build/Products" -maxdepth 2 -name 'Lists.app' -print -quit)"
if [[ -z "$APP" ]]; then
  echo "Build reported success but produced no Lists.app" >&2
  exit 1
fi
echo "OK — $APP"
